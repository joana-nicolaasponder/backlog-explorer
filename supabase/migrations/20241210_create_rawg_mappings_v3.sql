...
CREATE TABLE rawg_genre_mappings (
    ...
    genre_id UUID NOT NULL REFERENCES genres(id),  -- Changed to UUID to match genres table
...
CREATE TABLE rawg_platform_mappings (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    rawg_id INTEGER NOT NULL,
    platform_id UUID NOT NULL REFERENCES platforms(id),  -- Changed to UUID assuming platforms also uses UUID
    rawg_name TEXT NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL,
